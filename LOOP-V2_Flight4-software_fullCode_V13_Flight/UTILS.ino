...
//function call an I2C sensor (to read values after)
void callSensor4readings (uint8_t sensor, uint8_t adress){
  Wire.beginTransmission(sensor);
  Wire.write(adress);
  Wire.endTransmission(false);//keeps holding the bus to get all the bytes requested
}

//function to search in sensor (specify its adress in 2nd argument) + adress
void writeValueInside(uint8_t sensorAdress ,uint8_t adressValue, int16_t value){
