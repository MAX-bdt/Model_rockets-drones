...
const uint8_t paraAltitude = 20; //max altitude for parachute deployment (in meters)
const uint16_t paraDelay = 2000; //parachute deployment delay in seconds
//waiting state variables
unsigned long previousMillisWait=0;
unsigned long previousMillisFlight=0;
//-----CLOCKS--------------------------
unsigned long LoopTimer; //parameter that contains the length of each control loop (period)
unsigned long liftOffTime;
//SERVO VARIABLES
Servo gimbalX;
Servo gimbalZ;
Servo hatch;
const uint8_t pinServoX = 7;//D7 pin
const uint8_t AngleBaseX = 90;
const uint8_t pinServoZ = 8;//D8 pin
const uint8_t AngleBaseZ = 90;
const uint8_t pinServoHatch = 2;//D2 pin
const uint8_t AngleBaseHatch = 90;
//CHECKLIST VARIABLES
...
const uint8_t buzzer = 9;//D9 pin
const uint16_t waitInterval = 1000;
const uint16_t flightInterval = 150;
...
//JACK VARIABLES
const uint8_t jack = 4;//D4 pin
//SENSOR VARIABLES
//LIPO measure port variable
const uint8_t LIPO = 3;//A3 pin
//IMU variables
const uint8_t IMUadress = 0x68; //IMU sensor I2C ADRESS
float RateRoll, RatePitch, RateYaw;
float RateCalibrationRoll, RateCalibrationPitch, RateCalibrationYaw;
int RateCalibrationNumber;
float AccX, AccY, AccZ;
float AngleErrorRoll, AngleErrorPitch;
float AngleRoll=0;
float AnglePitch=0;
...
//BARO variables
const uint8_t BMP180adress = 0x77; //BARO sensor I2C ADRESS
short ac1;
short ac2;
short ac3;
short ac4;
...
const unsigned char OSS = 0;  // OverSampling Setting = precision setting
...
//SD VARIABLES
File dataFile;
const uint8_t pinSD = 10; //D10 >>> CS = Chip Select for SPI bus used by micro SD module
...
//INITILISATION TEST
float initialNUM = 1000; //number of values measured for initialisation of IMU & BARO
//---------------------PID VARIABLES---------------------------
...
float Tpid=0.004; //sets the delay in seconds for calculation of discret integrals in PID (here 0.004s>>>250Hz)
float MAXtvc=25;  //defines MAX (or MIN = -MAXtvc) for TVC servos range of mvt (>>> can be seen as saturation limit for a DC motor)
...
float PRateRoll=1 ; float PRatePitch=1;
float IRateRoll=0.02 ; float IRatePitch=0.02;
float DRateRoll=0.1 ; float DRatePitch=0.1;
//RQ : those variables will need specific tuning to be exactly suited for the loop rocket

//ADAPT THE LOOP FREQUENCY HERE :
float LoopMAXTime= 4000; //loop time in µs (microseconds)(here 4000µs = 0.004s -> 250Hz loop)
...
void IMUdataOK(){
  IMUsignals();
  if(RateRoll==0||RateYaw==0||RatePitch==0||AccX==0||AccY==0||AccZ==0){
    EXITstatus();
    }
}
