...
void SDinitialStates(){
  //fist row -> legend
  dataFile.print("batteryCharge_(%),IMUmoyRateRoll_(deg/s),IMUmoyRatePitch_(deg/s),IMUmoyRateYaw_(deg/s),IMUmoyAccX_(m/s-2),IMUmoyAccY_(m/s-2),IMUmoyAccZ_(m/s-2),BAROpressure_(hPa),BAROaltitude_(m),IMUangleErrorRoll_(deg),IMUangleErrorPitch_(deg),BAROinitialAltitude_(m)\n");
  dataFile.flush();
  //second row -> values
...
void writeSD(){
  ...
  dataFile.print(",");
  dataFile.print(RawCommandAngleRoll);
  dataFile.print(",");
  dataFile.print(RawCommandAnglePitch);
  dataFile.print("\n");
