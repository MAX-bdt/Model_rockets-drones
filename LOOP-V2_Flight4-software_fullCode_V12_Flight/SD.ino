...
void SDinitialStates(){
  //fist row -> legend
  dataFile.print("batteryCharge_(%),IMUmoyRateRoll_(deg/s),IMUmoyRatePitch_(deg/s),IMUmoyRateYaw_(deg/s),IMUmoyAccX_(g),IMUmoyAccY_(g),IMUmoyAccZ_(g),BAROpressure_(Pa),BAROaltitude_(m),IMUangleErrorRoll_(deg),IMUangleErrorPitch_(deg),BAROinitialAltitude_(m)\n");
  dataFile.flush();
  //second row -> values
...
void SDwriteLegend(){
  dataFile.print("Time_(ms),Altitude_(m),AngleRoll_(deg),AnglePitch_(deg),AngleRollCommand_(deg),AnglePitchCommand_(deg),AccNorm(g/1g=10m/s2),RotRateYaw(deg/s)\n");
  dataFile.flush();
}
...
void writeSD(){
  ...
  dataFile.print(",");
  dataFile.print(AccNorm);
  dataFile.print(",");
  dataFile.print(RateYaw); 
  dataFile.print("\n");
  //Flush counter ; flush each "flushloopsNb" = 80 loops (=each 1sec because of 80Hz loop)
